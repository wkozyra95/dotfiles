...
func release(ctx context.Context) error {
	...
	linuxAsset, darwinAsset, updateErr := updateAssets(client, release.GetID(), mycliLinux, mycliDarwin)
	if updateErr != nil {
		return updateErr
	}
	log.Infof("Linux asset %d", linuxAsset.GetID())
	log.Infof("MacOS asset %d", darwinAsset.GetID())
	return nil
}
